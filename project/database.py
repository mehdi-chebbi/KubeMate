...
def _get_encryption_key():
    ...
    key = os.environ.get('K8S_BOT_ENCRYPTION_KEY')
    if key:
        return key.encode()
    else:
        # Generate a new key and log warning
        key = Fernet.generate_key()
        logger.warning(f"No K8S_BOT_ENCRYPTION_KEY environment variable found. Generated new key: {key.decode()}")
        logger.warning("Set this environment variable to persist encryption across restarts!")
        return key

_cipher_suite = None

def get_cipher_suite():
    """Get singleton cipher suite instance"""
...
def encrypt_token(token: str) -> str:
    """Encrypt service account token using Fernet (AES-128 + HMAC)"""
    if not token:
        return None
...
def decrypt_token(encrypted_token: str) -> str:
    """Decrypt service account token using Fernet (AES-128 + HMAC)"""
...
class KubeconfigGenerator:
    """Generate kubeconfig files from service account credentials"""
    
    CONFIGS_DIR = Path(__file__).parent / "kubeconfigs"
    
    @classmethod
    def _ensure_configs_dir(cls):
        """Create kubeconfigs directory if it doesn't exist"""
        cls.CONFIGS_DIR.mkdir(parents=True, exist_ok=True)
    
    @classmethod
    def generate_filename(cls, name: str, config_id: int) -> str:
        """Generate unique filename from name and ID"""
        ...
        return f"{safe_name}-{config_id}.yaml"
    
    @classmethod
    def generate_kubeconfig(cls, config_id: int, name: str, 
                          cluster_url: str, token: str,
                          ca_cert: str = None, 
                          namespace: str = None) -> str:
        """
        Generate kubeconfig file from SA credentials
        
        Returns: Absolute path to generated file
        """
        cls._ensure_configs_dir()
        
        filename = cls.generate_filename(name, config_id)
        filepath = cls.CONFIGS_DIR / filename
        ...
        if ca_cert:
            kubeconfig_data['clusters'][0]['cluster']['certificate-authority-data'] = ca_cert
        
        # Add namespace if provided
        if namespace:
            kubeconfig_data['contexts'][0]['context']['namespace'] = namespace
        
        # Set current context
        kubeconfig_data['current-context'] = name
        
        # Write to file with secure permissions
        with open(filepath, 'w') as f:
            yaml.dump(kubeconfig_data, f, default_flow_style=False)
        
        # Set secure permissions (owner read-only)
        os.chmod(filepath, 0o600)
        
        logger.info(f"Generated kubeconfig file: {filepath}")
        return str(filepath)
    
    @classmethod
    def delete_config(cls, filepath: str):
        """Delete kubeconfig file"""
        if filepath and os.path.exists(filepath):
            os.remove(filepath)
            logger.info(f"Deleted kubeconfig file: {filepath}")
...
class Database:
    ...
    def _verify_api_keys_schema(self, cursor):
        ...
        required_columns = {'id', 'name', 'provider', 'api_key', 'endpoint_url', 'model', 'config',
                          'description', 'is_active', 'created_at', 'updated_at', 'created_by',
                          'last_used', 'usage_count', 'test_status', 'test_message', 'last_tested'}
        ...
        if missing:
            ...
            if 'endpoint_url' in missing:
                cursor.execute("""
                    ALTER TABLE api_keys 
                    ADD COLUMN endpoint_url VARCHAR(500)
                """)
                logger.info("Added endpoint_url column to api_keys table")
            ...
            if 'model' in missing:
                ...
                """)
                logger.info("Added model column to api_keys table")
            ...
            if 'config' in missing:
                ...
                """)
                logger.info("Added config column to api_keys table")
            
            if 'test_status' in missing:
                cursor.execute("""
                    ALTER TABLE api_keys 
                    ADD COLUMN test_status VARCHAR(50) DEFAULT 'untested'
                """)
                logger.info("Added test_status column to api_keys table")
            
            if 'test_message' in missing:
                cursor.execute("""
                    ALTER TABLE api_keys 
                    ADD COLUMN test_message TEXT
                """)
                logger.info("Added test_message column to api_keys table")
            
            if 'last_tested' in missing:
                cursor.execute("""
                    ALTER TABLE api_keys 
                    ADD COLUMN last_tested TIMESTAMP
                """)
                logger.info("Added last_tested column to api_keys table")
            
            # Make api_key nullable for local LLM providers
            if 'api_key' in missing:
                cursor.execute("""
                    ALTER TABLE api_keys 
                    ALTER COLUMN api_key DROP NOT NULL
                """)
                logger.info("Made api_key column nullable in api_keys table")
    ...
    def update_kubeconfig(self, kubeconfig_id: int, name: str = None, cluster_url: str = None,
        ...
        try:
            ...
            # Get existing config
            cursor.execute("SELECT * FROM kubeconfigs WHERE id = %s", (kubeconfig_id,))
    ...
    def create_llm_config(self, name: str, provider: str, api_key: str = None, 
                         endpoint_url: str = None, model: str = None, 
                         config: Dict[str, Any] = None, description: str = None,
                         created_by: int = None) -> Optional[int]:
        """Create a new LLM configuration (updated API key method)"""
        conn = None
        try:
            conn = self._get_connection()
            cursor = conn.cursor(cursor_factory=RealDictCursor)
            ...
            if provider == 'openrouter':
                model = model or 'minimax/minimax-01'
            elif provider == 'local':
                endpoint_url = endpoint_url or 'http://localhost:8080'
                model = model or 'default'
            
            cursor.execute("""
                INSERT INTO api_keys (name, provider, api_key, endpoint_url, model, 
                                  config, description, created_at, updated_at, created_by)
                VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s, %s)
                RETURNING id
            """, (name, provider, api_key, endpoint_url, model, 
                  json.dumps(config) if config else None, description, 
            ...
            if result:
                logger.info(f"Created LLM config '{name}' with provider '{provider}'")
                return result['id']
            else:
                return None
    ...
    def get_all_llm_configs(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # Parse JSON config for each result
            ...
            logger.error(f"Failed to get all LLM configs: {str(e)}")
            return []
        finally:
            if conn:
                cursor.close()
    ...
    def test_llm_config(self, config_id: int, test_result: Dict[str, Any]) -> bool:
        """Update test results for an LLM configuration"""
        conn = None
        try:
            conn = self._get_connection()
            cursor = conn.cursor(cursor_factory=RealDictCursor)
            ...
            if conn:
                conn.rollback()
            logger.error(f"Failed to update LLM config test results {config_id}: {str(e)}")
            return False
        finally:
            if conn:
                cursor.close()
    ...
    def set_active_llm_config(self, config_id: int) -> bool:
        """Set an LLM configuration as active (deactivate all others)"""
        conn = None
        try:
            conn = self._get_connection()
            cursor = conn.cursor(cursor_factory=RealDictCursor)
            ...
            if conn:
                conn.rollback()
            logger.error(f"Failed to set active LLM config {config_id}: {str(e)}")
            return False
        finally:
            if conn:
                cursor.close()
    ...
    def delete_llm_config(self, config_id: int) -> bool:
        ...
        try:
            ...
            cursor.execute("SELECT is_active FROM api_keys WHERE id = %s", (config_id,))
            result = cursor.fetchone()
            
            if result and result['is_active']:
                # For active configs, we'll allow deletion but first deactivate it
                logger.info(f"Deactivating active LLM config {config_id} before deletion")
                cursor.execute("UPDATE api_keys SET is_active = FALSE WHERE id = %s", (config_id,))
            
            cursor.execute("DELETE FROM api_keys WHERE id = %s", (config_id,))
            ...
            if conn:
                conn.rollback()
            logger.error(f"Failed to delete LLM config {config_id}: {str(e)}")
            return False
        finally:
            if conn:
                cursor.close()
