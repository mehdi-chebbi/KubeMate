...
const AdminDashboard = ({ user, onLogout }) => {
  ...
  const handleError = (errorMessage, type = 'error') => {
    if (type === 'success') {
      // For success messages, we could use a different state or just show as green error
    ...
    <div className="k8s-container min-h-screen">
      ...
      {error && (
        <div className="max-w-7xl mx-auto px-6 pt-4">
          <div className={`k8s-card p-4 flex items-center gap-3 ${
            error.includes('✅') 
            ...
            <span className={error.includes('✅') ? 'text-green-300' : 'text-red-300'}>
              {error}
            </span>
            <button
              onClick={() => setError('')}
              className={`ml-auto ${
                error.includes('✅') ? 'text-green-400 hover:text-green-300' : 'text-red-400 hover:text-red-300'
              }`}
            >
              ×
            </button>
